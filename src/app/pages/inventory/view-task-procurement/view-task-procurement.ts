...
@Component({
  ...
  imports: [DatePipe, EnumToStringPipe, LabelComponent, FormsModule, SelectWithSearch, FormField],
  templateUrl: './view-task-procurement.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ViewTaskProcurement {
  readonly dataService = inject(InventoryService);
  ...
  **/
  ...
  addFulfillmentGood(subTaskId: number, good: any) {
    const findSubTask = this.receivedTask
      .value()
      .tasksEntitiesProcurements.find((item) => item.id === subTaskId);
    if (!findSubTask) return;
    if (
      this.goodsBySubTask()[findSubTask.id] !== undefined &&
      this.goodsBySubTask()[findSubTask.id].length > findSubTask.remainingQuantity - 1
    ) {
      return;
    } else {
  ...
  summary = linkedSignal({
    ...
    computation: () => {
      const total = this.receivedTask
        .value()
        .tasksEntitiesProcurements.reduce((acc, val) => acc + val.quantity, 0);
      const remaining = this.receivedTask
        .value()
        .tasksEntitiesProcurements.reduce((acc, val) => acc + val.remainingQuantity, 0);
